...
export const notificationOutbox = pgTable(
  ...
  (t) => [
    ...
    check('notification_outbox_event_type_check', sql`${t.eventType} IN ('task.remind', 'task.due')`),
    check('notification_outbox_entity_type_check', sql`${t.entityType} IN ('task')`),
  ],
);
