...
import { asRecurrenceKind, asReminderMode, asReminderOffsetMinutes } from './task-dto.js';
import { getOwnedTaskOr404 } from './tasks.read.js';
import {
  dtoOf,
  fireIndexTask,
  fireRemoveTaskIndex,
...
export type TaskTx = Pick<Database, 'insert' | 'update' | 'delete' | 'select'>;

/** DB writes only. Caller owns the transaction and any post-commit index. */
export async function createTaskInTx(
  ...
  const row: NewTask = {
    ...
    recurrenceDtstart,
    completedAt: null,
    sortOrder: await nextSortOrder(listId, parentId, tx),
    deletedAt: null,
    createdAt: now,
    updatedAt: now,
  };
  ...
  const [created] = await tx.select().from(tasks).where(eq(tasks.id, row.id)).limit(1);
  if (!created) throw AppError.of(500, 'INTERNAL_ERROR');
  return created;
}

export async function createTask(userId: string, input: CreateTaskInput): Promise<Task> {
  const created = await getDb().transaction(async (tx) => createTaskInTx(tx, userId, input));
